...
Важно: использует multiprocessing, этим и отличается от postag_csv_dataset.py

TODO: оставить 1 из этих двух скриптов
"""
...
def process_text(text, mystem2upos_mapping):
    mystem_instance = Mystem()
    return custom_lemmatize_and_postag_with_mystem(
        text,
        mystem_instance,
...
def main():
    ...
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        df["text"] = list(
            tqdm(
                pool.imap(partial(process_text, mystem2upos_mapping=mystem2upos_mapping), df["text"]),
                total=len(df),
                desc="Lemmatizing and POS tagging"
            )
        )
